...
<task type="auto">
  ...
  <action>
    1. Create src/components/Sidebar.astro:
       ...
       - Render navigation tree recursively with GSD folder grouping
       - Tree nodes have {name, path, children} from buildNavTree
       - Render as nested ul/li: folders have button + nested ul, files have anchor links
       - Use recursive pattern or each loop with conditional for tree traversal
       - For GSD folders (phases/, research/, milestones/, todos/): render section label "GSD Structure" before group
