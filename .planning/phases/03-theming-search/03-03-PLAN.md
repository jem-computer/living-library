...
must_haves:
  truths:
    ...
    - "Search box in header accepts queries and shows live results"
    - "Search results display file name and context snippet"
...
<task type="auto">
  <name>Task 3: Add Pagefind integration to astro config</name>
...
Dev server should still start without errors.
  </verify>
  <done>
astro.config.mjs has pagefind integration configured as the last integration in the array.
  </done>
</task>
...
7. Search input visible in header (note: search results only work after build)
...
7. Verify search box appears in header
8. Verify code blocks switch themes correctly
9. (Optional) Run build and test search: `npm run build && npx serve dist`
  </how-to-verify>
  <resume-signal>Type "approved" if theming and search work correctly, or describe any issues</resume-signal>
</checkpoint>
